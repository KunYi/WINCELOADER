...
typedef struct BiosParameterBlockType3_t
{                                   //  Offset    Size
    char     versionId[8];          //  003         8
    uint8_t  mustBeZero[53];        //  00B        53
    uint64_t partitionOffset;       //  040         8
    uint64_t partitionLength;       //  048         8
    uint32_t fatOffset;             //  050         4
    uint32_t fatLength;             //  054         4
    uint32_t clusterHeapOffset;     //  058         4
    uint32_t clusterCount;          //  05C         4
    uint32_t firstClusterInRoot;    //  060         4
    uint32_t serialNumber;          //  064         4
    uint16_t fileSystemVersion;     //  068         2
    uint16_t extFlags;              //  06A         2
    uint8_t  bytesPerSector;        //  06C         1
    uint8_t  sectorsPerCluster;     //  06D         1
    uint8_t  numberOfFats;          //  06E         1
    uint8_t  driveId;               //  06F         1
    uint8_t  percentInUse;          //  070         1
    uint8_t  reserved[7];           //  071         7
} BiosParameterBlockType3_t;

typedef struct BiosParameterBlockTypeEx_t {
    char     versionId[8];                          // 003
    ...
    )
{
    ...
    BiosParameterBlockTypeEx_t *pBpbEx;
    DirEntry_t *pEntry;
    ...
    pBpbEx = (BiosParameterBlockTypeEx_t*)(IMAGE_XLDR_BOOTSEC_PA + 0x0003);
    ...
    else if (memcmp(pBpbEx->versionId, "EXFAT  ", sizeof(pBpbEx->versionId)-1) == 0) 
    {
        bytesPerSector = 1 << pBpbEx->bytesPerSectorLog2;
        sectorsPerCluster = 1 << pBpbEx->sectorsPerClusterLog2;
        sectors = pBpbEx->partitionOffset + pBpbEx->partitionLength;
        driveId = pBpbEx->driveId;
        fatSector = pBpbEx->fatOffset + pBpbEx->partitionOffset;
        if ((pBpbEx->extFlags & 0x01) && (pBpbEx->numberOfFats == 2))
            fatSector += pBpbEx->fatLength;
        // dirSector =
        // dataSector =
    ...
    {
        if (memcmp(&pSector[size], WCELDRNAME, 11) == 0)
            break;
        ...
        {
            BootLog(L"Couldn't find "WCELDRNAME" file!\r\n");   
            goto cleanUp;
        }
    }
    ...
    {
        if (!ReadSector(&s_fileSystem, sector))
            goto cleanUp;
        
        if (memcmp(pSector, "B000FF\x0A", 7) == 0)
            break;
    ...
    )
{
    ...
    while (size > 0)
    {
        ...
        {
            if (!ReadSector(pFileSystem, pFileSystem->readSector))
                goto cleanUp;
